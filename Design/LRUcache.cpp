...
// An LRU Cache evicts the least recently used item when the cache reaches its maximum capacity. 
// The two main operations required are:

// 	1.	get(key): Fetches the value of the key if the key exists in the cache, otherwise returns -1.
// 	2.	put(key, value): Inserts or updates the value if the key exists. When the cache reaches its capacity, 
...
//! DOUBLE LINKED LIST STL METHOD
class LRUCache {
public:
// list<int>::iterator is used to store adress of node of dll. in c++
list<int>dll;
map<int,pair<list<int>::iterator,int>>mpp; //key-{node_address,value}
int capacity;
    ...
    int get(int key) {
        if(!mpp.count(key))return -1;
            ...
            dll.erase(mpp[key].first); //erase that node 
            dll.push_front(key); //cache it
            mpp[key].first=dll.begin();// update node new address
        }
...
pthread_key_t key pair value : 0
make the PTHREAD_ONCE_INIT==90l
