...
//! : how Parent’s members are inherited into Child.
class student: protected human{//* All public members of human become protected inside student.
    int rollno,fees;
...
/* 
! DIAMOND PROBLEN (Hybrid inheritence) 
When a class inherits from two classes that both inherit from the same base class,
the derived class ends up with two copies of the base class, creating ambiguity.
...
? Why Ambiguity Occurs
	• D inherits A twice (once through B, once through C).
	• So compiler doesn’t know which A::show() to call —> from B or from C.

! Solution → Virtual Inheritance | we use virtual base class to overcome this problem
With virtual keyword, only one shared copy of Base class (A) exists in the inheritance hierarchy.
So D has one A, not two. This removes ambiguity and duplication.*/
...
int main(){
D obj;
obj.show(); // ambiguity solved by virtual
}
